...
const endOfShareDialog = code.indexOf('/>', shareDialogStart) + 2;

const codeBeforeShare = code.slice(0, shareDialogStart);
const codeAfterShare = code.slice(endOfShareDialog);
...
// wait, the original code had </Link> ))} at the end
// Let's just restore the file properly by re-reading it or looking at the chat history!
