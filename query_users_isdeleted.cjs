...
async function run() {
  const { data, error } = await supabase.from('users').select('id, isDeleted');
  console.log("Deleted count:", data?.filter(u => u.isDeleted === true).length);
  console.log("Non-deleted count:", data?.filter(u => u.isDeleted !== true).length);
}
run();
