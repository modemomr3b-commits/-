import { Search, ChevronLeft, Bell, Zap, TrendingUp, Clock, Filter, Layers } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const fetchCats = async () => {
       try {
         ...
         if (mounted) {
            // Keep only parent categories for the home page
            setCategories(cats.filter(c => !c.isHidden && !c.parentId).sort((a: any, b: any) => a.order - b.order));
            setLoading(false);
         }
       } catch (e) {
         console.error(e);
    ...
    <div className="p-4 md:p-8 space-y-8">
      ...
      >
        ...
        <div className="absolute left-3 top-1/2 -translate-y-1/2 flex gap-2">
           <div className="bg-white/10 px-2 py-1 rounded text-xs text-white/50 hidden md:block border border-white/5">Ctrl</div>
           <div className="bg-white/10 px-2 py-1 rounded text-xs text-white/50 hidden md:block border border-white/5">K</div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 relative rounded-3xl overflow-hidden bg-gradient-to-br from-brq-navy via-brq-royal to-purple-900 border border-brq-gold/30 lg:h-[320px] h-48 flex items-center shadow-2xl group cursor-pointer">
          ...
          >
            <span className="text-brq-gold text-sm md:text-base font-bold tracking-widest uppercase mb-2 block">— حصرياً في BRQ</span>
            <h2 className="text-2xl md:text-4xl lg:text-5xl font-black text-white mb-4 leading-tight">
              اكتشف أحدث <br/> تشكيلات <span className="text-transparent bg-clip-text bg-gradient-to-r from-brq-gold to-yellow-300">الأحذية</span>
            </h2>
            <div className="flex gap-4 items-center">
              <button className="bg-brq-gold text-black px-6 py-2.5 rounded-full font-bold hover:bg-yellow-400 transition-colors shadow-[0_0_15px_rgba(212,175,55,0.4)]">تسوق الآن</button>
              <p className="text-xs md:text-sm border border-white/20 px-4 py-2.5 rounded-full bg-black/40 backdrop-blur-md inline-flex items-center gap-2">
                <Zap size={14} className="text-yellow-400" /> خصم 30% لفترة محدودة
              </p>
            </div>
          </motion.div>
        </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-4">
          ...
          >
             ...
             <div>
               <p className="text-xs text-white/50 mb-1 uppercase tracking-wider font-mono">New Arrivals</p>
               <p className="font-bold text-lg">وصل حديثاً <span className="text-brq-gold">(50+)</span></p>
             </div>
          </motion.div>
          ...
          >
             ...
             <div>
               <p className="text-xs text-white/50 mb-1 uppercase tracking-wider font-mono">Trending Now</p>
          ...
          >
             ...
             <div>
               <p className="text-xs text-white/50 mb-1 uppercase tracking-wider font-mono">Last Updated</p>
      ...
      <div className="pt-6 border-t border-white/5">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link to="/search" className="text-sm text-brq-gold hover:text-white transition-colors flex items-center gap-1">عرض الكل <ChevronLeft size={14}/></Link>
        </div>
        
        {loading ? (
          <div className="flex justify-center p-12"><div className="w-10 h-10 border-4 border-brq-gold border-t-transparent rounded-full animate-spin shadow-[0_0_15px_rgba(212,175,55,0.5)]"></div></div>
        ) : categories.length === 0 ? (
          <div className="text-center p-12 glass-panel rounded-3xl text-white/50">لا توجد أقسام متاحة حالياً.</div>
        ) : (
              ...
              >
                ...
                >
                  ...
                  <div className="text-center">
                    <h3 className="font-bold text-lg group-hover:text-brq-gold transition-colors">{cat.name}</h3>
                    <p className="text-xs text-white/40 mt-1 flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">استعرض <ChevronLeft size={12}/></p>
                  </div>
                </Link>
              </motion.div>
            ))}
