...
import { Trash2, AlertTriangle, RefreshCcw, Loader2, Info, CheckSquare } from 'lucide-react';
import { api } from '../../api';

type CollectionType = 'products' | 'categories' | 'users' | 'orders' | 'updates';
...
export default function TrashManager() {
  ...
  const fetchTrash = async () => {
    ...
    try {
      ...
      setItems(data.sort((a: any, b: any) => (b.deletedAt || 0) - (a.deletedAt || 0)));
  ...
  const handleHardDelete = async (id?: string) => {
    ...
    if (!confirm('تحذير: سيتم حذف العنصر بشكل نهائي ولا يمكن التراجع عن هذا الإجراء! هل تريد المتابعة؟')) return;
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
         <div>
             ...
             <p className="text-sm text-white/50">يتم الاحتفاظ بالعناصر المحذوفة لمدة 30 يوماً قبل حذفها نهائياً</p>
         </div>
         {selectedItems.size > 0 && (
           <div className="flex items-center gap-2">
             <button onClick={() => handleRestore()} className="flex items-center gap-2 px-4 py-2 bg-emerald-500/20 text-emerald-400 border border-emerald-500/50 rounded-xl hover:bg-emerald-500/30 transition-colors font-bold text-sm">
                <RefreshCcw size={16} /> استعادة ({selectedItems.size})
             </button>
             <button onClick={() => handleHardDelete()} className="flex items-center gap-2 px-4 py-2 bg-red-500/20 text-red-500 border border-red-500/50 rounded-xl hover:bg-red-500/30 transition-colors font-bold text-sm">
      ...
      <div className="glass-panel border border-white/5 rounded-2xl overflow-hidden p-1">
        {loading ? (
          ...
          <div className="flex flex-col justify-center items-center h-64 text-center space-y-4">
             ...
             <p className="text-white/50">سلة المحذوفات فارغة (لا توجد {tabs.find(t=>t.id===activeTab)?.label} محذوفة)</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
              <table className="w-full text-sm text-right">
                 <thead className="bg-black/40 text-white/60">
                     <tr>
                       <th className="p-4 w-12 text-center rounded-tr-lg">
                         <button onClick={selectAll} className={`p-1 rounded ${selectedItems.size === items.length ? 'text-brq-gold' : 'text-white/30 hover:text-white/50'}`}>
                           <CheckSquare size={18} />
                         </button>
                       </th>
                 ...
                 <tbody className="divide-y divide-white/5 text-white/90">
                    {items.map((item) => (
                       <tr key={item.id} className={`transition-colors ${selectedItems.has(item.id) ? 'bg-brq-gold/5' : 'hover:bg-white/5'}`}>
                          ...
                          <td className="p-4">
                             <div className="font-bold text-white">
                                {item.name || item.username || item.title || item.orderNumber || (activeTab === 'updates' ? 'إشعار' : 'عنصر غير معروف')}
                             </div>
                             <div className="text-xs text-white/50 font-mono mt-1">ID: {item.id}</div>
                          </td>
                          <td className="p-4 text-white/60 text-xs" dir="ltr">
                             {item.deletedAt ? new Date(item.deletedAt).toLocaleString('ar-IQ') : 'غير معروف'}
                          </td>
                          ...
                          <td className="p-4">
                             <div className="flex gap-2">
                                <button onClick={() => handleRestore(item.id)} className="p-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 rounded-lg transition-colors text-xs font-bold flex items-center gap-1">
                                   <RefreshCcw size={14} /> استعادة
                                </button>
                                <button onClick={() => handleHardDelete(item.id)} className="p-2 bg-red-500/10 hover:bg-red-500/20 text-red-500 rounded-lg transition-colors text-xs font-bold flex items-center gap-1">
