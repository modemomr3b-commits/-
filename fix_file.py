...
# Remove all editingUser modals first
pattern = r"\{editingUser && \([\s\S]*?\}\)"
text = re.sub(pattern, "", text)

# There is a partial block around line 273: 
#                      <X size={20} />\n                  </button>\n               </div>\n               <form onSubmit={handleUpdate}...
# Let's just remove anything between `)}` (from the isAdding block) and `<div className="grid grid-cols-1 lg:grid-cols-4 gap-4">`
is_adding_end = text.find('</form>\n         </div>\n      )}\n')
if is_adding_end != -1:
    end_of_is_adding = is_adding_end + len('</form>\n         </div>\n      )}\n')
    grid_start = text.find('<div className="grid grid-cols-1 lg:grid-cols-4 gap-4">', end_of_is_adding)
    if grid_start != -1:
        text = text[:end_of_is_adding] + "      " + text[grid_start:]

# Now re-insert the editingUser modal right before the grid
modal_code = """{editingUser && (
...
grid_start2 = text.find('<div className="grid grid-cols-1 lg:grid-cols-4 gap-4">')
text = text[:grid_start2] + modal_code + text[grid_start2:]
